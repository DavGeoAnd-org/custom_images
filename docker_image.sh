...
otel-collector-service() {
  ...
  if [ "${env}" == "local" ]; then
    ...
    docker push "$REGISTRY/$REPOSITORY:$OTEL_COLLECTOR_CONTRIB_VERSION"
...
otel-java-agent() {
  JAVA_VERSION=$(yq '.java.version' ./otel-java-agent/version.yaml)
  OTEL_JAVA_AGENT_VERSION=$(yq '.otel-java-agent.version' ./otel-java-agent/version.yaml)
  ...
  if [ "${env}" == "local" ]; then
    docker build --no-cache -t local/java-otel-agent:"${JAVA_VERSION}"-"${OTEL_JAVA_AGENT_VERSION}" -t local/java-otel-agent:latest --build-arg java_version="${JAVA_VERSION}" ./otel-java-agent/
...
if [ "${image}" == "otel-collector-service" ]; then
  otel-collector-service "$@"
elif [ "${image}" == "otel-java-agent" ]; then
  otel-java-agent
else
  echo "Please select an image: otel-collector-service, otel-java-agent"
fi